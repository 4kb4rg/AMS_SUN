...
Public Class BD_trx_WSMechHourDist_Distribute : Inherits Page
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            If Not Page.IsPostBack Then
                ...
                lblWorkCode.Text = Request.QueryString("Workcode")
                BindDistList()
                BindGrid()
            End If
    ...
    Sub onload_GetLangCap()
        GetEntireLangCap()
        lblTitle.Text = "WORKSHOP MECHANIC HOUR CALENDERISATION"
    ...
    Protected Function LoadTotal() As Decimal
        ...
        strLoadTotalCond = "| AND BD.LocCode = '" & Trim(strLocation) & "'" & _
                           " AND BD.EmpCode = '" & lblEmpCode.Text & "'" & _
                           " AND BD.WorkCode = '" & lblWorkCode.Text & "'" & _
                           " AND BD.PeriodID = " & GetActivePeriod("")
        ...
        If GetBudgetPeriod() = 6 Then 
            ...
            decTotalAddVote = RunSPReturnValue(objSqlConn, "BD_CLSTRX_WSMECHHOURDIST_SUM_ADDVOTE_GET", _
                                               "@EmpCode", lblEmpCode.Text, _
                                               "@WorkCode", lblWorkCode.Text, _
        ...
        Else
            Try
                intErrNo = objBD.mtdGetMechanicHour("BD_CLSTRX_WSMECHHOUR_LIST_GET", strLoadTotalCond, dsTotals)
            ...
            lblDistFig.Text = objGlobal.GetIDDecimalSeparator(FormatNumber(Trim(dsTotals.Tables(0).Rows(0).Item("MechHour")), 0))
            Return Trim(dsTotals.Tables(0).Rows(0).Item("MechHour"))
        End If
    End Function

    Protected Function LoadData() As DataSet
        Dim strOppCd_WSMechHourDist_AccPeriod_GET As String = "BD_CLSTRX_WSMECHHOURDIST_ACCPERIOD_GET"
        strParam = "|" & strLocation & "|" & GetActivePeriod("") & "||||"

        Try
            intErrNo = objBD.mtdGetWSMechHourDist(strOppCd_WSMechHourDist_AccPeriod_GET, strParam, objDataSet)
    ...
    Sub btnConfirm_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Dim decTotal As Decimal = CDec(lblDistFig.Text)
        Dim decRunningNum As Decimal = decTotal
        Dim decFigure As Decimal

        Dim txt As TextBox
        Dim lblMonth As Label
        Dim lblYear As Label

        Dim strOpCodeUpdHour As String = "BD_CLSTRX_WSMECHHOURDIST_ACCPERIOD_HOUR_UPD"
        Dim strOpCodeUpdAddVote As String = "BD_CLSTRX_WSMECHHOURDIST_ACCPERIOD_ADDVOTE_UPD"
        ...
        For intCnt = 0 To objDataSet.Tables(0).Rows.Count - 1
            ...
            If intCnt = objDataSet.Tables(0).Rows.Count - 1 Then 
                decFigure = decRunningNum
            Else
                If CInt(ddlDistribute.SelectedItem.Value) = objBD.EnumDistMethod.Percentage Then
                    decFigure = decTotal * CDec(txt.Text.Trim) / 100
                Else
                    decFigure = CDec(txt.Text.Trim)
                End If

                decRunningNum = decRunningNum - decFigure
            End If

            Try
                intErrNo = objBD.mtdUpdWSMechHourDist(strOpCodeUpdHour, strOpCodeUpdAddVote, _
                                                      intBgtPeriod, decFigure, lblMonth.Text.Trim, _
                                                      lblYear.Text.Trim, GetActivePeriod(""), _
                                                      lblEmpCode.Text, lblWorkCode.Text, _
        ...
        If intErrNo = 0 Then
            Response.Write("<Script Language=""JavaScript"">opener.location.href='BD_trx_WSMechHourDist_Details.aspx?EmpCode=" & lblEmpCode.Text & "&WorkCode=" & lblWorkCode.Text & "';window.close();</Script>")
        End If
