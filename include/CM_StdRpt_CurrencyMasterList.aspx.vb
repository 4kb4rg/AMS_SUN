...
Public Class CM_StdRpt_CurrencyMasterList : Inherits Page
    ...
    Protected WithEvents txtCurrencyCode as TextBox
    ...
    Sub BindStatusList()
        ...
        lstStatus.Items.Add(New ListItem(objCMSetup.mtdGetCurrencyStatus(objCMSetup.EnumCurrencyStatus.All), objCMSetup.EnumCurrencyStatus.All))
        lstStatus.Items.Add(New ListItem(objCMSetup.mtdGetCurrencyStatus(objCMSetup.EnumCurrencyStatus.Active), objCMSetup.EnumCurrencyStatus.Active))
        lstStatus.Items.Add(New ListItem(objCMSetup.mtdGetCurrencyStatus(objCMSetup.EnumCurrencyStatus.Deleted), objCMSetup.EnumCurrencyStatus.Deleted))
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Try
            intErrNo = objSysCfg.mtdGetConfigInfo(strParam, _
                                                  ...
                                                  objSysCfgDs)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=WM_STDRPT_Currency_MASTER_GET_CONFIG_DATE&errmesg=" & lblErrMessage.Text & "&redirect=../en/reports/WM_StdRpt_Selection.aspx")
        End Try
        ...
        If Not (strDateFrom = "" And strDateTo = "") Then
           ...
           Response.Write("<Script Language=""JavaScript"">window.open(""CM_StdRpt_CurrencyMasterListPreview.aspx?Type=Print&Location=" & strUserLoc & "&RptID=" & strRptID & "&RptName=" & strRptName & "&Decimal=" & "" & _
                           "&DateFrom=" & objDateFrom & "&DateTo=" & objDateTo & "&CurrencyCode=" & strCurrencyCode  & "&Name=" & strName & "&Telephone=" & strTelephone & "&UpdatedBy=" & strUpdatedBy  & _
