...
Public Class BD_MatureCrop_Det : Inherits Page

    Protected WithEvents dgMatureCropDet As DataGrid
    ...
    Dim strOppCd_MatureCrop_CostPerWeight_SUM As String
    Dim strOppCd_MatureCrop_CostPerArea_SUM As String
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            If Not Page.IsPostBack Then
                ...
                lblSubBlkCode.Text = Request.QueryString("subblk") 
    ...
    Sub onload_GetLangCap()
        ...
        dgMatureCropDet.Columns(1).HeaderText = GetCaption(objLangCap.EnumLangCap.Account) & lblCode.Text
    End Sub
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_MATURECROP_DET_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_MatureCrop_Details.aspx")
        End Try
    ...
    Sub GetAreaStmtTotalArea()
        ...
        If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
            strOppCd_MatureCrop_CostPerArea_SUM = "BD_CLSTRX_MATURECROP_BLKTOTALAREA_GET"
            strParam = objGLSetup.EnumBlockType.MatureField & "|" & objGLSetup.EnumBlockStatus.Active & "|" & strLocation & "|" & hidBlkCode.Value & "|"
        Else
            strOppCd_MatureCrop_CostPerArea_SUM = "BD_CLSTRX_MATURECROP_SUBBLKTOTALAREA_GET"
            strParam = objGLSetup.EnumSubBlockType.MatureField & "|" & objGLSetup.EnumSubBlockStatus.Active & "|" & strLocation & "|" & hidBlkCode.Value & "|"
        End If

        Try
            intErrNo = objBD.mtdGetMatureCropTotalArea(strOppCd_MatureCrop_CostPerArea_SUM, strParam, dsTotalArea)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_MATURECROPDET_GET_AREASTMT_TOTALAREA&errmesg=" & lblErrMessage.Text & "&redirect=BD/trx/BD_trx_MatureCrop_Details.aspx")
        End Try


        lblTotalAreaFig.Text = objGlobal.GetIDDecimalSeparator(FormatNumber(dsTotalArea.Tables(0).Rows(0).Item("AreaSize"), 0))
    ...
    Sub GetTotalProd()
        ...
        If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
            strOppCd_MatureCrop_CostPerWeight_SUM = "BD_CLSTRX_MATURECROP_PRODFFB_GET"
        Else
            If Request.QueryString("DistByBlk") = True Then
                strOppCd_MatureCrop_CostPerWeight_SUM = "BD_CLSTRX_MATURECROP_SBLKYIELD_DISTBYBLOCK_PRODFFB_GET"
            Else
                strOppCd_MatureCrop_CostPerWeight_SUM = "BD_CLSTRX_MATURECROP_PRODFFB_GET"
            End If
        End If
        ...
        Try
            intErrNo = objBD.mtdGetMatureCropTotalFFB(strOppCd_MatureCrop_CostPerWeight_SUM, strParam, dsTotalFFB)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_CROPPROD_TOTALFFB&errmesg=" & lblErrMessage.Text & "&redirect=BD/trx/BD_trx_MatureCrop_Details.aspx")
        End Try
    ...
    Sub BindGrid()
        ...
        dgMatureCropDet.DataSource = LoadData()
        dgMatureCropDet.DataBind()
    ...
    Protected Function LoadData() As DataSet
        ...
        strParam = "|" & strLocation & "|" & GetActivePeriod("") & "|" & hidBlkCode.Value & "||MCS.DispSeq ASC"
        Try
            intErrNo = objBD.mtdGetMatureCrop(strOppCd_MatureCrop_Format_GET, strParam, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_MATURECROP_DET_GET&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_MatureCrop_Details.aspx")
        End Try
    ...
    Protected Function GetActivePeriod(ByRef BGTPeriod As String) As String
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_BUDGETACTIVEPERIODS&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_MatureCrop_Details.aspx")
        End Try
    ...
    Sub DEDR_Edit(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        ...
        dgMatureCropDet.EditItemIndex = CInt(E.Item.ItemIndex)
        BindGrid()
    End Sub
    ...
    Sub DEDR_Update(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        ...
        strParam = GetActivePeriod("") & "|" & _
                   strMatureCropSetID & "|" & _
                   strFreq & "|" & _
                   strUnit & "|" & _
                   strUnitCost & "|" & _
                   objBD.EnumMatureCropStatus.Budgeted & "|" & _
                   strDisp & "|" & _
                   strDispCol & "|" & _
                   strMandays & "|" & _
                   hidBlkCode.Value & "|" & _
        ...
        Try
            intErrNo = objBD.mtdUpdMatureCrop(strOppCd_MatureCrop_Format_GET, _
                                            ...
                                            False)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=MATURECROP_UPD&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_MatureCrop_Details.aspx")
        End Try
        ...
        Else
            For intEdit = E.Item.ItemIndex + 1 To dgMatureCropDet.Items.Count - 1
                label = dgMatureCropDet.Items.Item(CInt(intEdit)).FindControl("lblDispType")
                If label.Text.Trim <> objBDSetup.EnumBudgetFormatItem.Header Then
                    Exit For
                End If
            Next

            dgMatureCropDet.EditItemIndex = intEdit
            BindGrid()
        End If
    End Sub

    Sub DEDR_Cancel(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        If CInt(E.Item.ItemIndex) = 0 And dgMatureCropDet.Items.Count = 1 And dgMatureCropDet.PageCount <> 1 Then
            dgMatureCropDet.CurrentPageIndex = dgMatureCropDet.PageCount - 2
        End If
        dgMatureCropDet.EditItemIndex = -1
        BindGrid()
    End Sub
    ...
    Sub Recalc_Formula()
        ...
        strParam = "|" & strLocation & "|" & GetActivePeriod("") & "|" & hidBlkCode.Value & "|MC.MatureCropSetID ASC"
        Try
            intErrNo = objBD.mtdUpdMatureCropFormula(strParam, _
                                                    strOppCd_MatureCrop_Format_GET, _
                                                    strOppCd_MatureCrop_ADD, _
                                                    strOppCd_MatureCropSetup_GET, _
                                                    strOppCd_MatureCrop_UPD, _
                                                    strOpCd_Formula_GET, _
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_MATURECROP_DET_RECALC_FORM_GET&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_MatureCrop_Details.aspx")
        End Try
        BindGrid()
