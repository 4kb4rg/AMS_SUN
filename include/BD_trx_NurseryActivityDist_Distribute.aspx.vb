...
Public Class BD_NurseryActivity_Dist : Inherits Page
    ...
    Protected WithEvents lblNurseryActDistID As Label
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            lblNurseryActDistID.Text = Request.QueryString("id")
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=BD_TRX_NURSERYACTIVITYDIST_DISTRIBUTE_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_NurseryActivityDist_Details.aspx")
        End Try
    ...
    Protected Function LoadTotal() As Decimal
        Dim strOppCd_NurseryActivityDist_YrBudgetCost_SUM As String
        Dim dsTotals As DataSet
        Dim decTotalAddVote As Decimal
        Dim strLoadTotalCond As String

        If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
            strOppCd_NurseryActivityDist_YrBudgetCost_SUM = "BD_CLSTRX_NURSERYACTIVITYDIST_YRBUDGETCOST_SUM"
            strLoadTotalCond = "|" & strLocation & "|" & _
                               GetActivePeriod("") & "|" & hidBlkCode.Value & "|AND NAS.AccCode = '" & lblAccCode.Text & "'|"
        Else
            If hidDistByBlk.Value = True Then
                strOppCd_NurseryActivityDist_YrBudgetCost_SUM = "BD_CLSTRX_NURSERYACTIVITYDIST_YRBUDGETCOST_SUM_SBLK"
                strLoadTotalCond = "|" & strLocation & "|" & GetActivePeriod("") & "||AND NAS.Acccode = '" & lblAccCode.Text & "' " & _
        ...
        If GetBudgetPeriod() = 6 Then 
            ...
            decTotalAddVote = RunSPReturnValue(objSqlConn, "BD_CLSTRX_NURSERYACTIVITY_SUM_ADDVOTE_GET", _
        ...
        Else
            Try
                intErrNo = objBD.mtdGetNurseryActivity(strOppCd_NurseryActivityDist_YrBudgetCost_SUM, strLoadTotalCond, dsTotals)
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_NURSERYACTIVITYDIST_LOADTOTALS&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_NurseryActivityDist_Details.aspx")
            End Try
    ...
    Protected Function LoadData() As DataSet
        ...
        Try
            intErrNo = objBD.mtdGetNurseryActivityDist(strOppCd_NurseryActivityDist_AccPeriod_GET, strParam, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_NURSERYACTIVITYDIST&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_NurseryActivityDist_Details.aspx")
        End Try
    ...
    Sub btnConfirm_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        Dim strOppCd_NurseryActivityDist_YrBudgetCost_SUM As String
        Dim strOppCd_NurseryActivityDist_ID_GET As String = "BD_CLSTRX_NURSERYACTIVITYDIST_ID_GET"
        Dim strOppCd_NurseryActivityDist_AccPeriod_GET As String = "BD_CLSTRX_NURSERYACTIVITYDIST_ACCPERIOD_GET"
        ...
        For intCnt = 0 To objDataSet.Tables(0).Rows.Count - 1
            ...
            Select Case ddlDistribute.SelectedItem.Value
                Case objBD.EnumDistMethod.Even
                    If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
                        strOppCd_NurseryActivityDist_YrBudgetCost_SUM = "BD_CLSTRX_NURSERYACTIVITYDIST_YRBUDGETCOST_SUM"
                        strParam = GetActivePeriod("") & "|" & lblNurseryActDistID.Text & "|" & txt.Text.Trim & "||" & lblAccCode.Text.Trim & "|" & lblMonth.Text.Trim & "|" & lblYear.Text.Trim & "|" & hidBlkCode.Value & "|"
                    Else
                        If hidDistByBlk.Value = True Then
                            strOppCd_NurseryActivityDist_YrBudgetCost_SUM = "BD_CLSTRX_NURSERYACTIVITYDIST_YRBUDGETCOST_SUM_SBLK"
            ...
            End Select
            Try
                intErrNo = objBD.mtdUpdNurseryActivityDist(strOppCd_NurseryActivityDist_AccPeriod_UPD, _
                                                           strOppCd_NurseryActivityDist_UPD, _
                                                           strOppCd_NurseryActivityDist_YrBudgetCost_SUM, _
                                                           strOppCd_NurseryActivityDist_ID_GET, _
                                                           strOppCd_NurseryActivityDist_AccPeriod_GET, _
                                                           strCompany, _
                                                           strLocation, _
                                                           strUserId, _
                                                           strParam, _
            ...
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_NURSERYACTIVITYDIST_CONFIRM&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_NurseryActivityDist_Details.aspx")
            End Try
        ...
        If intErrNo = 0 Then
            Response.Write("<Script Language=""JavaScript"">opener.location.href='BD_trx_NurseryActivityDist_Details.aspx?DistByBlk=" & hidDistByBlk.Value & "&blkcode=" & lblBlkCode.Text & "&subblkcode=" & lblSubBlkCode.Text & "&acccode=" & lblAccCode.Text & "';window.close();</Script>")
        End If
    End Sub
