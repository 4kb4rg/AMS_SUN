...
Public Class BD_ImMatureCrop_Dist : Inherits Page
    ...
    Protected WithEvents lblUnMatureCropDistID As Label
    ...
    Dim strOppCd_UnMatureDist_UPD As String = "BD_CLSTRX_UNMATURECROPDIST_UPD"
    Dim strOppCd_UnMatureDist_AccPeriod_UPD As String = "BD_CLSTRX_UNMATURECROPDIST_ACCPERIOD_UPD"
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            lblUnMatureCropDistID.Text = Request.QueryString("id").Trim
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("/include/mesg/ErrorMessage.aspx?errcode=BD_TRX_IMMATURECROPDIST_DISTRIBUTE_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_ImMatureCropDist_Details.aspx")
        End Try
    ...
    Protected Function LoadTotal() As Decimal
        Dim strOppCd_UnMatureCropDist_YrBudgetCost_SUM As String
        Dim dsTotals As DataSet
        Dim decTotalAddVote As Decimal
        Dim strLoadTotalCond As String

        If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
            strOppCd_UnMatureCropDist_YrBudgetCost_SUM = "BD_CLSTRX_UNMATURECROPDIST_YRBUDGETCOST_SUM"
            strLoadTotalCond = "|" & strLocation & "|" & _
                               GetActivePeriod("") & "|" & hidBlkCode.Value & "|AND UMCS.AccCode = '" & lblAccCode.Text & "'|"
        Else
            If hidDistByBlk.Value = True Then
                strOppCd_UnMatureCropDist_YrBudgetCost_SUM = "BD_CLSTRX_UNMATURECROPDIST_YRBUDGETCOST_SUM_SBLK"
                strLoadTotalCond = "|" & strLocation & "|" & GetActivePeriod("") & "||AND UMCS.Acccode = '" & lblAccCode.Text & "' " & _
                                   "AND SBLK.BlkCode = '" & lblBlkCode.Text & "'|"
            Else
                strOppCd_UnMatureCropDist_YrBudgetCost_SUM = "BD_CLSTRX_UNMATURECROPDIST_YRBUDGETCOST_SUM"
                strLoadTotalCond = "|" & strLocation & "|" & _
                                   GetActivePeriod("") & "|" & hidBlkCode.Value & "|AND UMCS.AccCode = '" & lblAccCode.Text & "'|"
        ...
        If GetBudgetPeriod() = 6 Then 
            ...
            decTotalAddVote = RunSPReturnValue(objSqlConn, "BD_CLSTRX_IMMATURECROPDIST_SUM_ADDVOTE_GET", _
        ...
        Else
            Try
                intErrNo = objBD.mtdGetUnMatureCrop(strOppCd_UnMatureCropDist_YrBudgetCost_SUM, strLoadTotalCond, dsTotals)
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_IMMATURECROPDIST_LOADTOTALS&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_ImMatureCropDist_Details.aspx")
            End Try
    ...
    Protected Function LoadData() As DataSet
        ...
        Try
            intErrNo = objBD.mtdGetUnMatureCropDist(strOppCd_UnMatureCropDist_AccPeriod_GET, strParam, objDataSet)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_IMMATURECROPDIST&_LOADDATAerrmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_ImMatureCropDist_Details.aspx")
        End Try
    ...
    Sub btnConfirm_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        Dim strOppCd_UnMatureCropDist_YrBudgetCost_SUM As String
        Dim strOppCd_UnMatureCropDist_ID_GET As String = "BD_CLSTRX_UNMATURECROPDIST_ID_GET"
        Dim strOppCd_UnMatureCropDist_AccPeriod_GET As String = "BD_CLSTRX_UNMATURECROPDIST_ACCPERIOD_GET"
        ...
        For intCnt = 0 To objDataSet.Tables(0).Rows.Count - 1
            ...
            Select Case ddlDistribute.SelectedItem.Value
                Case objBD.EnumDistMethod.Even
                    If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
                        strOppCd_UnMatureCropDist_YrBudgetCost_SUM = "BD_CLSTRX_UNMATURECROPDIST_YRBUDGETCOST_SUM"
                        strParam = GetActivePeriod("") & "|" & lblUnMatureCropDistID.Text & "|" & txt.Text.Trim & "||" & lblAccCode.Text.Trim & "|" & lblMonth.Text.Trim & "|" & lblYear.Text.Trim & "|" & hidBlkCode.Value & "|"
                    Else
                        If hidDistByBlk.Value = True Then
                            strOppCd_UnMatureCropDist_YrBudgetCost_SUM = "BD_CLSTRX_UNMATURECROPDIST_YRBUDGETCOST_SUM_SBLK"
            ...
            End Select
            Try
                intErrNo = objBD.mtdUpdUnMatureCropDist(strOppCd_UnMatureDist_AccPeriod_UPD, _
                                                        strOppCd_UnMatureDist_UPD, _
                                                        strOppCd_UnMatureCropDist_YrBudgetCost_SUM, _
                                                        strOppCd_UnMatureCropDist_ID_GET, _
                                                        strOppCd_UnMatureCropDist_AccPeriod_GET, _
                                                        strCompany, _
                                                        strLocation, _
                                                        strUserId, _
                                                        strParam, _
                                                        interror, _
                                                        hidDistByBlk.Value, intBgtPeriod)
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_IMMATURECROPDIST_CONFIRM&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_ImMatureCropDist_Details.aspx")
            End Try
        ...
        If intErrNo = 0 Then
            Response.Write("<Script Language=""JavaScript"">opener.location.href='BD_trx_ImMatureCropDist_Details.aspx?DistByBlk=" & hidDistByBlk.Value & "&blkcode=" & lblBlkCode.Text & "&subblkcode=" & lblSubBlkCode.Text & "&acccode=" & lblAccCode.Text & "&yr=" & lblYearPlanted.Text & "';window.close();</Script>")
        End If
