...
Public Class CM_StdRpt_PriceBasisMasterListPreview : Inherits Page
    ...
    Dim objCMPetup As New agri.CM.clsSetup()   
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Session("SS_PriceBasisCode") = Request.QueryString("PriceBasisCode")
    ...
    Sub BindReport()
        ...
        Dim strOpCdPR_GET As String = "CM_STDRPT_PriceBasis_MASTER_LIST"


        If Not (Request.QueryString("DateFrom") = "" And Request.QueryString("DateTo") = "") Then
            SearchStr = searchStr & " AND (DateDiff(Day, '" & Session("SS_DATEFROM") & "', CMP.UpdateDate) >= 0) And (DateDiff(Day, '" & Session("SS_DATETO") & "', CMP.UpdateDate) <= 0) "
        End If


        If Request.QueryString("Status") = "All" Then
            SearchStr = SearchStr & "AND CMP.Status LIKE '%' "
        ElseIf Request.QueryString("Status") = "Active" Then
            SearchStr = SearchStr & "AND CMP.Status = '" & objCMPetup.EnumPriceBasisStatus.Active & "'"
        ElseIf Request.QueryString("Status") = "Deleted" Then
            SearchStr = SearchStr & "AND CMP.Status = '" & objCMPetup.EnumPriceBasisStatus.Deleted & "'"
        End If

        If Not Request.QueryString("PriceBasisCode") = "" Then
            SearchStr = SearchStr & " AND CMP.PriceBasisCode LIKE '" & Request.QueryString("PriceBasisCode") & "'"
        Else
            SearchStr = SearchStr & " AND CMP.PriceBasisCode LIKE '%' "
        End If

        If Not Request.QueryString("Name") = "" Then
            SearchStr = SearchStr & " AND CMP.description LIKE '" & Request.QueryString("Name") & "'"
        Else
            SearchStr = SearchStr & " AND CMP.description LIKE '%' "
        End If


        If Not Request.QueryString("UpdatedBy") = "" Then
            SearchStr = SearchStr & " AND CMP.UpdateID LIKE '" & Request.QueryString("UpdatedBy") & "'"
        Else
            SearchStr = SearchStr & " AND CMP.UpdateID LIKE '%' "
        End If
        ...
        Try
            intErrNo = objCM.mtdGetReport_PriceBasisMasterList(strOpCdPR_GET, strParam, objRptDs, objMapPath)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=GET_WM_PriceBasis_MASTER_LIST_REPORT&errmesg=" & lblErrMessage.Text & "&redirect=")
        End Try



        rdCrystalViewer.Load(objMapPath & "web\" & strLangCode & "\Reports\Crystal\CM_StdRpt_PriceBasisMasterList.rpt", OpenReportMethod.OpenReportByTempCopy)
        ...
        crDiskFileDestinationOptions.DiskFileName = objMapPath & "web\ftp\CM_StdRpt_PriceBasisMasterList.pdf"
        ...
        Response.Write("<META HTTP-EQUIV=""refresh"" CONTENT=""0; URL=../../ftp/CM_StdRpt_PriceBasisMasterList.pdf"">")
    End Sub
    ...
    Sub PassParam()
        ...
        paramField9 = paramFields.Item("ParamPriceBasisCode")
        ...
        ParamDiscreteValue9.Value = Session("SS_PRICEBASISCODE")
