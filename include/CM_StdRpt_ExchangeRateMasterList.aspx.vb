...
Public Class CM_StdRpt_ExchangeRateMasterList : Inherits Page
    ...
    Sub BindStatusList()        
        lstStatus.Items.Add(New ListItem(objCMSetup.mtdGetExchangeRateStatus(objCMSetup.EnumExchangeRateStatus.All), objCMSetup.EnumExchangeRateStatus.All))
        lstStatus.Items.Add(New ListItem(objCMSetup.mtdGetExchangeRateStatus(objCMSetup.EnumExchangeRateStatus.Active), objCMSetup.EnumExchangeRateStatus.Active))
        lstStatus.Items.Add(New ListItem(objCMSetup.mtdGetExchangeRateStatus(objCMSetup.EnumExchangeRateStatus.Deleted), objCMSetup.EnumExchangeRateStatus.Deleted))
    End Sub
    ...
    Sub btnPrintPrev_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Try
            intErrNo = objSysCfg.mtdGetConfigInfo(strParam, _
                                                  ...
                                                  objSysCfgDs)
        Catch Exp As System.Exception
            Response.Redirect("../../include/mesg/ErrorMessage.aspx?errcode=WM_STDRPT_ExchangeRate_MASTER_GET_CONFIG_DATE&errmesg=" & lblErrMessage.Text & "&redirect=../en/reports/WM_StdRpt_Selection.aspx")
        End Try
        ...
        If Not (strDateFrom = "" And strDateTo = "") Then
            If objGlobal.mtdValidInputDate(strDateSetting, strDateFrom, objDateFormat, objDateFrom) = True And objGlobal.mtdValidInputDate(strDateSetting, strDateTo, objDateFormat, objDateTo) = True Then            
                Response.Write("<Script Language=""JavaScript"">window.open(""CM_StdRpt_ExchangeRateMasterListPreview.aspx?Type=Print&RptID=" & strRptID & "&RptName=" & strRptName & _
                                "&DateFrom=" & objDateFrom & "&DateTo=" & objDateTo & "&Status=" & strStatus & """,null ,""status=yes, resizable=yes, scrollbars=yes, toolbar=no, location=no"");</Script>")
            Else
