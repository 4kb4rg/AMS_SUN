...
Public Class BD_ImMatureCropDist_Details : Inherits Page

    Protected WithEvents dgImMatureCropDist As DataGrid
    ...
    Sub Page_Load(ByVal Sender As Object, ByVal E As EventArgs)
        ...
        Else
            ...
            If SortExpression.Text = "" Then
                SortExpression.Text = "UMCD.AccCode"
                SortCol.Text = "ASC"
            End If
    ...
    Sub onload_GetLangCap()
        ...
        dgImMatureCropDist.Columns(0).HeaderText = GetCaption(objLangCap.EnumLangCap.Account) & lblCode.Text
    End Sub
    ...
    Sub GetEntireLangCap()
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_IMMATURECROPDIST_GET_LANGCAP&errmesg=" & lblErrMessage.Text & "&redirect=BD/Setup/BD_trx_ImMatureCropDist_Details.aspx")
        End Try
    ...
    Sub BindGrid()
        ...
        dgImMatureCropDist.DataSource = LoadData()
        LoadTotal()
        dgImMatureCropDist.DataBind()
        lblLocCode.Text = strLocation
        GetActivePeriod(Period)
        lblBgtPeriod.Text = Period
        Formatting()

        dgImMatureCropDist.Items.Item(dgImMatureCropDist.Items.Count - 1).Font.Bold = True
    End Sub
    ...
    Sub Formatting()
        ...
        EditButton = dgImMatureCropDist.Items.Item(CInt(dgImMatureCropDist.Items.Count - 1)).FindControl("Edit")
    ...
    Protected Function GetActivePeriod(ByRef BGTPeriod As String) As String
        ...
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_IMMATURECROPDIST_GET_BUDGETACTIVEPERIODS&errmesg=" & lblErrMessage.Text & "&redirect=BD/trx/BD_trx_ImMatureCropDist_Details.aspx")
        End Try
    ...
    Private Sub InitializeBoundColumns()
        Dim strOppCd_UnMatureCropDist_AccPeriod_GET As String = "BD_CLSTRX_UNMATURECROPDIST_ACCPERIOD_GET"
        ...
        Try
            intErrNo = objBD.mtdGetUnMatureCropDist(strOppCd_UnMatureCropDist_AccPeriod_GET, _
                                                    strParam, _
                                                    dsPeriod)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_IMMATURECROPDIST_ACCPERIOD_GET&errmesg=" & lblErrMessage.Text & "&redirect=BD/Trx/BD_trx_ImMatureCropDist_Details.aspx")
        End Try
        ...
        For intCntPeriod = 0 To dsPeriod.Tables(0).Rows.Count - 1
            ...
            dgImMatureCropDist.Columns.AddAt(intCntIns, bcInsert)
            ...
            dgImMatureCropDist.Columns.AddAt(intAddVoteCol, tmpcolAddVote)
        Next
    ...
    Protected Function LoadData() As DataSet
        Dim strOppCd_UnMatureCropDist_Format_GET As String
        Dim dsYrBudgetCost As New DataSet()
        Dim strParamCost As String
        Dim blnIsTotal As Boolean

        If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
            strOppCd_UnMatureCropDist_Format_GET = "BD_CLSTRX_UNMATURECROPDIST_FORMAT_GET"
        ...
        Else
            If Request.QueryString("distbyblk") = True Then
                strOppCd_UnMatureCropDist_Format_GET = "BD_CLSTRX_UNMATURECROPDIST_FORMAT_SBLK_GET"
        ...
        Try
            intErrNo = objBD.mtdGetUnMatureCropDistFormat(strOppCd_UnMatureCropDist_Format_GET, _
                                                          ...
                                                          blnIsTotal)
        Catch Exp As System.Exception
            Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=GET_IMMATURECROPDIST_DET&errmesg=" & lblErrMessage.Text & "&redirect=BD/trx/BD_trx_ImMatureCropDist_Details.aspx")
        End Try
    ...
    Protected Sub LoadTotal()
        Dim strOppCd_UnMatureCropDist_Total_SUM As String
        Dim dsTotalCost As New DataSet()
        Dim intCnt As Integer
        Dim intDRCnt As Integer

        If objDataSet.Tables(0).Rows.Count > 0 And Not IsNothing(objDataSet) Then
            dr = objDataSet.Tables(0).NewRow()
            dr("UnMatureCropDistID") = 0
            dr("BlkCode") = ""
            dr("YearPlanted") = ""
            dr("AccCode") = "TOTAL"
            dr("AccDesc") = ""
            ...
            If objSysCfg.mtdHasConfigValue(objSysCfg.mtdGetConfigSetting(objSysCfg.EnumConfig.BlockCostLevel), intConfigsetting) = True Then
                strOppCd_UnMatureCropDist_Total_SUM = "BD_CLSTRX_UNMATURECROPDIST_TOTAL_SUM"
                strParam = strLocation & "|" & GetActivePeriod("") & "|||" & hidBlkCode.Value & "||"
            Else
                If Request.QueryString("distbyblk") = True Then
                    strParam = strLocation & "|" & GetActivePeriod("") & "|||| AND SBLK.BlkCode = '" & lblBlkCode.Text & "'|"
                    strOppCd_UnMatureCropDist_Total_SUM = "BD_CLSTRX_UNMATURECROPDIST_TOTAL_SUM_SBLK"
                Else
                    strOppCd_UnMatureCropDist_Total_SUM = "BD_CLSTRX_UNMATURECROPDIST_TOTAL_SUM"
                    strParam = strLocation & "|" & GetActivePeriod("") & "|||" & hidBlkCode.Value & "||"
                End If
            ...
            Try
                intErrNo = objBD.mtdGetUnMatureCropDistFormat(strOppCd_UnMatureCropDist_Total_SUM, _
                                                            strOppCd_Period_GET, _
                                                            strParam, _
                                                            dsTotalCost, _
                                                            intError, _
                                                            True)
            Catch Exp As System.Exception
                Response.Redirect("../../../include/mesg/ErrorMessage.aspx?errcode=BD_TRX_IMMATURECROPDIST_DET_LOADTOTAL_GET&errmesg=" & lblErrMessage.Text & "&redirect=BD/trx/BD_trx_ImMatureCropDist_Details.aspx")
            End Try
    ...
    Private Sub AddTotalAddVoteCol(ByRef objDS As DataSet)
        ...
        For i = 0 To objDS.Tables(0).Rows.Count - 1
            With objDS.Tables(0).Rows(i)
                .Item("TotalAddVote") = RunSPReturnValue(objSqlConn, "BD_CLSTRX_IMMATURECROPDIST_SUM_ADDVOTE_GET", _
    ...
    Sub DEDR_Edit(ByVal Sender As Object, ByVal E As DataGridCommandEventArgs)
        ...
        label = E.Item.FindControl("lblUnMatureCropDistID")
        strID = label.Text
        label = E.Item.FindControl("lblAccCode")
        strAccCode = label.Text

        BindGrid()
        Response.Write("<Script Language=""JavaScript"">pop_Dist=window.open(""../../BD/Trx/BD_Trx_ImMatureCropDist_Distribute.aspx?blkcode=" & lblBlkCode.Text & "&subblkcode=" & lblSubBlkCode.Text & _
               "&id=" & strID.Trim & "&DistByBlk=" & Request.QueryString("distbyblk") & "&acccode=" & strAccCode.Trim & "&yr=" & lblYearPlanted.Text & _
